 ...
 * language is actually installed on the target runner(s) — see `docs/TTS_VOICE_SETUP.md`.
 */
...
export class TtsHelper {
     ...
     * voice bridged into the classic SAPI5 hive (`TTS_VOICE_SETUP.md`) synthesizes real Devanagari
    ...
    static generateWavBase64(text: string, lang?: AppLanguage): string {
        ...
        const selectVoice = culture
            ? `$v = $s.GetInstalledVoices() | Where-Object { $_.VoiceInfo.Culture.Name -eq '${culture}' } | Select-Object -First 1; `
              + `if (-not $v) { throw 'No installed SAPI5 voice for culture ${culture} - see docs/TTS_VOICE_SETUP.md' }; `
            ...
            throw new Error(
                ...
                (culture
                    ? `Voice selection for culture '${culture}' ran but still produced silence — ` +
                      `the installed voice may not cover this exact text. See docs/TTS_VOICE_SETUP.md.`
